...
public abstract class AbstractInstanceRegistry implements InstanceRegistry {
    ...
    // 读写锁用来控制对recentlyChangedQueue的访问,防止获取增量数据是频繁变动
     ...
     *
     * 4个地方会修改:
     *  1. Eureka-Server 初始化 initEurekaServerContext#openForTraffic
     *  2. 定时重置 scheduleRenewalThresholdUpdateTask
     *  3. 注册 PeerAwareInstanceRegistryImpl#register 增加 numberOfRenewsPerMinThreshold 、expectedNumberOfRenewsPerMin
     *  4. 下线 PeerAwareInstanceRegistryImpl#cancel 减少 numberOfRenewsPerMinThreshold 、expectedNumberOfRenewsPerMin
     */
    /**
     * 期望最小每分钟续租次数
     */
    ...
    protected AbstractInstanceRegistry(EurekaServerConfig serverConfig, EurekaClientConfig clientConfig, ServerCodecs serverCodecs) {
        ...
        // 台任务定时顺序扫描队列，当 lastUpdateTime 超过一定时长后进行移除。
    ...
    public void register(InstanceInfo registrant, int leaseDuration, boolean isReplication) {
        try {
            ...
            // 已经注册过,使用数据不一致的时间大的应用注册信息为有效的
            if (existingLease != null && (existingLease.getHolder() != null)) {
                // 假设两次注册请求后一次比前一次先到达,后一次到达的请求不会覆盖前一次的
            ...
            // 设置应用实例信息的操作类型为添加，并添加到最近租约变更记录队列(recentlyChangedQueue )。
            // recentlyChangedQueue 用于注册信息的增量获取
        ...
        try {
            ...
            if (lease == null) {
                ...
                if ((info != null) && !(info.getStatus().equals(newStatus))) {
                    ...
                    // 设置 应用实例状态
                    // 设置后，Eureka-Client 拉取注册信息，被更新覆盖状态的应用实例就是设置的状态。
        ...
        try {
            ...
            if (lease == null) {
                ...
                if (currentOverride != null && info != null) {
                    // 设置 应用实例覆盖状态,用于 Eureka-Server 集群同步
    ...
    public void evict(long additionalLeaseMs) {
        ...
        // 计算 清理租约数量 ,一次清理过期租约个数不允许超过全部的的85%
    ...
    public Applications getApplicationsFromMultipleRegions(String[] remoteRegions) {
        ...
        // 计算应用集合 hashcode, 变量用于校验增量获取的注册信息和 Eureka-Server 全量的注册信息是否一致
    ...
    protected void postInit() {
        ...
        evictionTimer.schedule(evictionTaskRef.get(),
                // 清理租约过期任务执行频率，单位：毫秒。默认，60000 毫秒。
